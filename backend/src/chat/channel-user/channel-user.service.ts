...
export class ChannelUserService {
	...
	async findChannelUserByChannelIdAndUserId(channel_id: number, user_id: number) {
		return await this.channelUsersRepository.findOne({
			relations: ['user', 'channel'],
			where: {
				channel: { channel_uid: channel_id },
	...
	async getMutedOfUserByUserIdAndChannelId(user_id: number, channel_id: number) {
		const channel_user =  await this.channelUsersRepository.findOne({
			where: {
				user: {id: user_id},
				channel: {channel_uid: channel_id}
	...
	async updateMuteUserByUserIdAndChannelId(user_id: number, channel_id: number, muted_days: number, user: User) {
		try {
				if (!user)
					throw new ForbiddenException(
						'Not logged in',
					);
				const channelOwner = await this.findChannelUserByChannelIdAndUserId(channel_id, user.id);
				if (!channelOwner) {
					throw new ForbiddenException(
						'User not in channel',
					);
				...
				const channelUser = await this.findChannelUserByChannelIdAndUserId(channel_id, user_id);
				if (!channelUser) {
					throw new ForbiddenException(
						'User to be updated not in channel',
					);
				}
				let date = new Date();
				...
				if (muted_days == 0)
					date = null;
				return await this.channelUsersRepository.update(channelUser.channeluser_uid, {
					mutedUntil: date
				})
		} catch (error) {
	...
	async unmuteUserByUserIdAndChannelId(user_id: number, channel_id: number) {
		try {
				...
				let date = new Date(channelUser.mutedUntil);
	...
	async updateStatus(status: string, user_id: number, channel_id: number, user: User) {
		try {
			if (status === Status.Null || status === Status.Muted || status === Status.Banned) {
	...
	async updateRole(role: string, user_id: number, channel_id: number, user: User) {
		try {
			if (role === Role.Admin || role === Role.Owner || role === Role.User) {
				const newStatus: Role = role;
				if (!user)
					throw new ForbiddenException(
						'Not logged in',
					);
				...
				throw new ForbiddenException(
					'Input Valid Role, {null, banned, muted}',
				);
			}
		} catch (error) {
			console.log('error=', error.message);
	...
	async deleteAllChannelUserByChannelIdAndUserId(channel_uid: number, user_id: number) {
		return await this.channelUsersRepository.delete({
			channel: {channel_uid: channel_uid},
			user: {id: user_id}
		});
